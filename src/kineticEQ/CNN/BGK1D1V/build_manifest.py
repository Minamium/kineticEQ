...
def _split_case_ids(case_ids: List[int], seed: int, ratios: Tuple[float, float, float]) -> Dict[str, List[int]]:
    """
...
def build_manifest(
    ...
    """
    Build manifest JSON:
      - files: list of {path, meta, n_steps}
    ...
    for p in files:
        try:
            ...
            rec = {
                ...
                "nx": int(meta["nx"]),
                "n_steps": int(n_steps),
                "meta": meta,  # keep full meta for future filtering
            }
            records.append(rec)
            ...
            if strict:
                # strict=False なら無視して続行、strict=Trueなら後でまとめて失敗扱いにする
                pass
...
def main():
    ...
    ap.add_argument("--data_root", type=str, default="mgpu_output", help="Directory containing shard_rank*/case_*.npz")
    ap.add_argument("--out", type=str, default="mgpu_output_manifest.json", help="Output manifest json")
    ap.add_argument("--seed", type=int, default=0)
    ap.add_argument("--train", type=float, default=0.8)
    ap.add_argument("--val", type=float, default=0.1)
    ap.add_argument("--test", type=float, default=0.1)
    ap.add_argument("--strict", action="store_true", help="If set, keep bad_files list but do not fail early.")
    args = ap.parse_args()
    ...
    manifest = build_manifest(
        ...
        seed=args.seed,
        ratios=ratios,
        strict=not args.strict,  # default: strict mode in our sense = keep going anyway
    )
    ...
    print(f"  split sizes  : train={len(manifest['splits']['train'])}, val={len(manifest['splits']['val'])}, test={len(manifest['splits']['test'])}")
