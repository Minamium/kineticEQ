# kineticEQ/analysis/BGK1D1V/plotting/plot_moment_cnn_test_v2.py
...
def _get_legacy_case_root(d: dict, case_index: int = 0) -> tuple[dict, dict, dict]:
...
def _parse_case_name_meta(case_name: str) -> tuple[float | None, float | None]:
    tau = None
    dt = None
    m_tau = re.search(r"(?:^|_)tau([0-9p.eE+-]+)(?:_|$)", str(case_name))
    m_dt = re.search(r"(?:^|_)dt([0-9p.eE+-]+)(?:_|$)", str(case_name))
    if m_tau:
        tau = _as_float(m_tau.group(1).replace("p", "."), default=None)
    if m_dt:
        dt = _as_float(m_dt.group(1).replace("p", "."), default=None)
    return tau, dt


def _parse_spec_meta(spec_name: str) -> tuple[float | None, str | None]:
    picard_tol = None
    conv_type = None
    m_tol = re.search(r"(?:^|_)tol_([0-9p.eE+-]+)(?:_|$)", str(spec_name))
    m_conv = re.search(r"(?:^|_)eval_([a-zA-Z]+)(?:_|$)", str(spec_name))
    if m_tol:
        picard_tol = _as_float(m_tol.group(1).replace("p", "."), default=None)
    if m_conv:
        conv_type = str(m_conv.group(1))
    return picard_tol, conv_type


def _legacy_extract_record(path: Path, case_index: int = 0) -> CaseRecord:
    d = _load_json(path)
    meta, case, detail = _get_legacy_case_root(d, case_index=case_index)
...
def _eval_report_extract_record(path: Path, case_index: int = 0) -> CaseRecord:
    d = _load_json(path)
    run_results = d.get("run_results", [])
    comparisons = d.get("comparisons", [])
    report_meta = d.get("meta", {}) if isinstance(d.get("meta"), dict) else {}
    spec_name = str(d.get("spec_name", ""))

    if not isinstance(run_results, list) or not run_results:
        raise ValueError(f"{path}: JSON has no run_results.")
    if not isinstance(comparisons, list) or not comparisons:
        raise ValueError(f"{path}: JSON has no comparisons.")

    case_names = sorted(
        {
            str(r.get("case_name"))
            for r in run_results
            if isinstance(r, dict) and r.get("case_name") is not None
        }
    )
    if not case_names:
        raise ValueError(f"{path}: run_results contain no case_name.")
    if not (0 <= case_index < len(case_names)):
        raise ValueError(f"{path}: case_index={case_index} out of range: cases size={len(case_names)}")

    case_name = case_names[int(case_index)]
    case_comps = [c for c in comparisons if isinstance(c, dict) and str(c.get("case_name")) == case_name]
    if not case_comps:
        raise ValueError(f"{path}: no comparison entry found for case {case_name!r}.")
    if len(case_comps) != 1:
        raise ValueError(
            f"{path}: selected case {case_name!r} has {len(case_comps)} comparison entries; "
            "plot_moment_cnn_test_v2 expects exactly one baseline-target pair per input report."
        )
    comp = case_comps[0]

    baseline_name = str(comp.get("baseline_name"))
    target_name = str(comp.get("target_name"))
    base_candidates = [
        r for r in run_results
        if isinstance(r, dict)
        and str(r.get("case_name")) == case_name
        and str(r.get("target_name")) == baseline_name
    ]
    warm_candidates = [
        r for r in run_results
        if isinstance(r, dict)
        and str(r.get("case_name")) == case_name
        and str(r.get("target_name")) == target_name
    ]
    if len(base_candidates) != 1 or len(warm_candidates) != 1:
        raise ValueError(
            f"{path}: could not resolve unique baseline/warm run_results for case={case_name!r}, "
            f"baseline={baseline_name!r}, target={target_name!r}."
        )

    base = base_candidates[0]
    warm = warm_candidates[0]
    base_meta = base.get("meta", {}) if isinstance(base.get("meta"), dict) else {}
    warm_meta = warm.get("meta", {}) if isinstance(warm.get("meta"), dict) else {}

    tau_tilde, dt = _parse_case_name_meta(case_name)
    spec_tol, conv_type = _parse_spec_meta(spec_name)
    picard_tol = _as_float(
        report_meta.get("picard_tol", base_meta.get("picard_tol", warm_meta.get("picard_tol", spec_tol))),
        default=None,
    )
    if picard_tol is None:
        raise ValueError(f"{path}: could not infer picard_tol from EvalReport.")

    gpu_name = report_meta.get("gpu_name", None)
    if not isinstance(gpu_name, str):
        gpu_name = None
    if gpu_name is None:
        gpu_name = str(report_meta.get("device", warm_meta.get("device", base_meta.get("device", "unknown"))))
    if conv_type and gpu_name == "unknown":
        gpu_name = f"unknown/{conv_type}"

    def _step_array(run: dict, key: str, *, dtype: Any) -> np.ndarray:
        steps = run.get("step_records", [])
        if not isinstance(steps, list):
            return np.asarray([], dtype=dtype)
        vals = [rec.get(key) for rec in steps if isinstance(rec, dict)]
        if key == "infer_time_sec":
            vals = [float("nan") if v is None else v for v in vals]
        return np.asarray(vals, dtype=dtype)

    t_base = _step_array(base, "walltime_sec", dtype=np.float64)
    t_warm = _step_array(warm, "walltime_sec", dtype=np.float64)
    it_base = _step_array(base, "picard_iter", dtype=np.int64)
    it_warm = _step_array(warm, "picard_iter", dtype=np.int64)
    if t_base.size == 0 or t_warm.size == 0 or it_base.size == 0 or it_warm.size == 0:
        raise ValueError(f"{path}: EvalReport step_records are missing/empty.")
    ...
    t_inf_est = _step_array(warm, "infer_time_sec", dtype=np.float64)[:n_steps]
    if t_inf_est.size > 0:
        valid = t_inf_est[~np.isnan(t_inf_est)]
        mean_infer_time_est = float(np.mean(valid)) if valid.size > 0 else float("nan")
    else:
    ...
    sum_base = int(comp.get("picard_sum_base", int(np.sum(it_base[it_base > 0]))))
    sum_warm = int(comp.get("picard_sum_target", int(np.sum(it_warm[it_warm > 0]))))
    speedup = float(sum_base) / float(max(sum_warm, 1))

    fm_b = base.get("final_moments", {})
    fm_w = warm.get("final_moments", {})
    if not isinstance(fm_b, dict) or not isinstance(fm_w, dict):
        raise ValueError(f"{path}: EvalReport final_moments missing or not dict.")
    ...
    T_warm = np.asarray(fm_w.get("T", []), dtype=np.float64)
    if n_base.size == 0 or n_warm.size == 0:
        raise ValueError(f"{path}: EvalReport final_moments arrays are missing/empty.")
...
def _extract_record(path: Path, case_index: int = 0) -> CaseRecord:
    ...
    if isinstance(d.get("cases"), list):
        return _legacy_extract_record(path, case_index=case_index)
    if isinstance(d.get("run_results"), list) and isinstance(d.get("comparisons"), list):
        return _eval_report_extract_record(path, case_index=case_index)
    raise ValueError(f"{path}: unsupported JSON schema for plot_moment_cnn_test_v2.")
