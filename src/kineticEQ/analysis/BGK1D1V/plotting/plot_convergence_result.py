# kineticEQ/src/kineticEQ/analysis/BGK1D1V/plotting/plot_convergence_result.py
...
def plot_convergence_results(
    ...
    出力ファイル（filename をベース名として扱う）:
      - <base>.png             : Figure 1（HOLO vs Picard: iter/residual）
      - <base>_holo.png        : Figure 2（HOLO-only: outer/LO inner）
      - <base>_walltime.png    : Figure 3（walltime: linear/log）
    ...
    filename : str
        出力ファイルのベース名。拡張子省略時は .png を付与。
        例: "test_cvg_BGK1D_cuda.png" または "test_cvg_BGK1D_cuda"
    output_dir : str | None
        出力先ディレクトリ。None の場合はカレント。
    dt, nx, nv, ho_tol, picard_tol, lo_tol : optional
        図タイトル用の注釈。None の場合 meta から補完。
    figsize : tuple
        各 Figure のサイズ
    show_plots : bool
        True のとき表示（plt.show）
    ...
    for tau in taus:
        ...
        if info["time_ho"]:
            ...
            if info["wall_ho"]:
                ...
                # walltime は time_ho と同じサンプル点数である前提
                t_wall_norm = t_norm[: len(w_ho)]
                mark_every_w = max(1, len(t_wall_norm) // 20)
    ...
    ax_outer.set_ylabel("Iterations per time step")
    ax_outer.set_title("Iteration count")
    ...
    ax_outer_ho.set_title("HOLO outer iterations")
    ...
    ax_lo_inner.set_title("LO inner iterations (HOLO)")
    ...
    if gpu_name:
        fig3.suptitle(f"Per-step walltime – Device: {gpu_name}", fontsize=12)
    else:
        fig3.suptitle("Per-step walltime", fontsize=12)
    ...
    f1 = out_dir / f"{base}{ext}"
    f2 = out_dir / f"{base}_holo{ext}"
    f3 = out_dir / f"{base}_walltime{ext}"

    fig1.savefig(str(f1), dpi=300, bbox_inches="tight")
    fig2.savefig(str(f2), dpi=300, bbox_inches="tight")
    fig3.savefig(str(f3), dpi=300, bbox_inches="tight")
