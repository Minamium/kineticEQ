...
class BGK1DPlotMixin:
    ...
    def plot_distribution_interactive(
        ...
        """
        style="volume" で f と |f−f_ref| を半透明ボリュームとして表示。
        そのほかの引数・戻り値は従来と同じ。
        """
        import numpy as np
        from scipy.interpolate import interp1d
        ...
        # ───── 入力検証・キー決定（略） ─────
        # ※ ここは前回版と同じロジックを使う
        # ...（bench_dict, ref_result, target などを用意）...
        ...
        for k in target:
            ...
            fig.add_trace(
                go.Volume(
                    ...
                    z=np.zeros(nx*nv),
                    value=f_vol.ravel(),
                    isomin=f_vol.min(), isomax=f_vol.max(),
                    opacity=0.8,
                    surface_count=15,
                    colorscale="Viridis",
                    showscale=False,
            ...
            fig.update_layout(
                title=f"Grid {k} (nx×nv={nx}×{nv}) – volume",
            ...
            # 保存・表示
            if save_html:
                fn = f"{fname_base}_grid{k}_volume.html"
                fig.write_html(fn)
                saved.append(fn)
            if show_plots:
                fig.show()
