...
class BGK1D:
    ...
    # ============================================================
    # HOLO 用: LO フラックス構築ヘルパー
    # ============================================================
    def _build_LO_flux_from_moments(self,
                                    n: torch.Tensor,
                                    u: torch.Tensor,
                                    T: torch.Tensor,
                                    Q: torch.Tensor) -> torch.Tensor:
        """
        (n,u,T,Q) から LO 形式の界面フラックス F_{i+1/2} を構成するヘルパー。
        形式は論文どおり
        ...
        入力:
            n,u,T,Q : shape = (nx,)
        出力:
            F_half : shape = (nx-1, 3)  （i+1/2 のフラックスベクトル）
        """
        dtype  = self.dtype
        device = self.device
        ...
        # 質量フラックス F1
        nu_half = 0.5 * (nu[:-1] + nu[1:])  # (nx-1,)

        # 運動量フラックス F2 = nu^2/n + P
        mom_flux_center = nu * nu / (n + 1e-30) + P         # (nx,)
        mom_flux_half   = 0.5 * (mom_flux_center[:-1] +
                                 mom_flux_center[1:])       # (nx-1,)

        # エネルギーフラックス F3 = (U + P) u + Q
        energy_flux_center = (U + P) * u + Q                # (nx,)
        energy_flux_half   = 0.5 * (energy_flux_center[:-1] +
                                    energy_flux_center[1:])  # (nx-1,)

        F_half = torch.empty((self.nx - 1, 3), dtype=dtype, device=device)
        F_half[:, 0] = nu_half
        F_half[:, 1] = mom_flux_half
        F_half[:, 2] = energy_flux_half
        return F_half

    # ============================================================
    # HOLO 用: フラックス整合 γ の診断メソッド
    # ============================================================
    @torch.no_grad()
    def flux_consistency_diagnostics(self,
                                     f_ho: torch.Tensor,
                                     n_lo: torch.Tensor,
                                     u_lo: torch.Tensor,
                                     T_lo: torch.Tensor,
        ...
        """
        γ が「本来補正したいフラックス差」とどれだけ一致しているかを診断する。

        入力
        ----
        f_ho : (nx, nv)
            HO 系の分布（任意の z-ステージ）.
            ここから S^{HO}, n^{HO,new}, u^{HO,new}, T^{HO,new} を作る。
        n_lo, u_lo, T_lo : (nx,)
            LO 系 Picard 解から得られたモーメント。
            （_LO_calculate_moments(...) の戻り値をそのまま渡せばよい。）
        Q_ho : (nx,)
            HO 系の熱流束 Q^HO_i （_HO_calculate_fluxes(f_ho) の結果）。
        dt_for_gamma : float or None
            γ を定義するときに使った Δt。通常 self.dt と同じなので、
            None なら self.dt を使用。
        print_summary : bool
            True のとき、誤差の要約を print する。
        ...
        result : dict
            {
              ...
              "y_gamma_over_dt":  Tensor (nx-2,3),   # γ 由来の y_i / Δt
              "div_diff_flux":    Tensor (nx-2,3),   # (F_LO(LO) - S_HO)' の離散化
              "gamma_face":       Tensor (nx-1,3),   # γ_{i+1/2}
              "F_LO_from_HO":     Tensor (nx-1,3),   # f_ho のモーメントから作った LO 形式フラックス
              "F_LO_from_LO":     Tensor (nx-1,3),   # LO 解から作った LO 形式フラックス
              "S_face":           Tensor (nx-1,3),   # HO upwind モーメント S^{HO}
            }
        """
        ...
        if dt_for_gamma is None:
            dt_for_gamma = self.dt

        # ---- 1. HO 側: upwind モーメントフラックス S^{HO}_{i+1/2} ----
        S_1_face, S_2_face, S_3_face = self._HO_calculate_moments(f_ho)  # (nx-1,)
        S_face = torch.stack((S_1_face, S_2_face, S_3_face), dim=1)      # (nx-1,3)

        # ---- 2. HO 分布 f_ho から "新しい" HO モーメントを計算 ----
        n_ho_new, u_ho_new, T_ho_new = self.calculate_moments(f_ho)

        # HO モーメントから LO 形式フラックス F_LO^{HO} を構成
        F_LO_from_HO = self._build_LO_flux_from_moments(
            n_ho_new, u_ho_new, T_ho_new, Q_ho
        )   # (nx-1,3)

        # ---- 3. γ_face を構成: γ_{i+1/2} = -S^{HO}_{i+1/2} + F_LO^{HO}_{i+1/2} ----
        gamma_face = -S_face + F_LO_from_HO                           # (nx-1,3)

        # セル中心の y_i = (γ_{i+1/2} - γ_{i-1/2}) / Δx
        if nx <= 2:
            raise RuntimeError("nx <= 2 では内部セルが無いので診断できません。")
        y_gamma = (gamma_face[1:, :] - gamma_face[:-1, :]) / dx        # (nx-2,3)
        y_gamma_over_dt = y_gamma / dt_for_gamma                       # γ 由来の y_i/Δt

        # ---- 4. LO 解から見た「本当に補正したいフラックス差」 ----
        # F_LO(LO) を LO 解 (n_lo,u_lo,T_lo) から構成
        F_LO_from_LO = self._build_LO_flux_from_moments(
            n_lo, u_lo, T_lo, Q_ho
        )   # (nx-1,3)

        # 「HO upwind モーメント S_HO」と「LO 解のフラックス F_LO(LO)」の差
        diff_flux = F_LO_from_LO - S_face                              # (nx-1,3)

        # その発散 (F_LO(LO) - S_HO)' ≒ (差の前後差)/Δx
        div_diff_flux = (diff_flux[1:, :] - diff_flux[:-1, :]) / dx    # (nx-2,3)

        # ---- 5. γ が狙っているものとの誤差 ----
        err_y   = div_diff_flux - y_gamma_over_dt                      # (nx-2,3)
        err_y_max = float(torch.max(torch.abs(err_y)))
        err_y_l2  = float(torch.sqrt(torch.mean(err_y ** 2)))

        # 単純に F_LO(LO) と F_LO(HO) の差も見ておく
        err_F       = F_LO_from_LO - F_LO_from_HO
        err_F_max   = float(torch.max(torch.abs(err_F)))
        err_F_l2    = float(torch.sqrt(torch.mean(err_F ** 2)))

        errors = {
            "max_err_y":     err_y_max,
            "l2_err_y":      err_y_l2,
            "max_err_flux":  err_F_max,
            "l2_err_flux":   err_F_l2,
        }

        if print_summary:
            print("=== Flux consistency diagnostics ===")
            print(f"  max |(F_LO(LO) - S_HO)' - y_gamma/dt| = {err_y_max:.3e}")
            print(f"   L2 |(F_LO(LO) - S_HO)' - y_gamma/dt| = {err_y_l2:.3e}")
            print(f"  max |F_LO(LO) - F_LO(HO)|            = {err_F_max:.3e}")
            print(f"  L2 |F_LO(LO) - F_LO(HO)|             = {err_F_l2:.3e}")

        result = {
            "errors":           errors,
            "y_gamma_over_dt":  y_gamma_over_dt.detach().cpu(),
            "div_diff_flux":    div_diff_flux.detach().cpu(),
            "gamma_face":       gamma_face.detach().cpu(),
            "F_LO_from_HO":     F_LO_from_HO.detach().cpu(),
            "F_LO_from_LO":     F_LO_from_LO.detach().cpu(),
            "S_face":           S_face.detach().cpu(),
        }
        return result
